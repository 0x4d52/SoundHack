...
unsigned char short2alawtab[16384] = {
...
{
	unsigned char alaw_dude;
	
	alaw_dude = short2alawtab[(pcm_val / 4) & 0x3fff];
	return(alaw_dude);
}
...
float	Alaw2Float(unsigned char a_val)
{
	...
	floatsam = (float)alaw2shorttab[a_val];
	return(floatsam);
}
...
void	Alaw2ShortBlock(unsigned char *aBlock, short *shortBlock, long size)
{
	...
	for(i=0;i<size;i++)
		*(shortBlock+i) = alaw2shorttab[*(aBlock+i)];
}
